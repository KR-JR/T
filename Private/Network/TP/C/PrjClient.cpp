...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	static BOOL bDrawing = FALSE, bEraseMode = FALSE; // 지우개 모드 추가
	...
	case WM_MOUSEMOVE:
		if(bDrawing && g_bStart){
			...
			if(bEraseMode) {
				// 지우개 모드
				hBrush = CreateSolidBrush(GetSysColor(COLOR_WINDOW)); // 시스템 배경색 사용
				hOldBrush = (HBRUSH)SelectObject(hDCMem, hBrush);

                // 작은 사각형으로 지우개 효과 구현
                Rectangle(hDCMem, x0 - 5, y0 - 5, x0 + 5, y0 + 5);
